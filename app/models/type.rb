...
class Type < ApplicationRecord
  ...
  def self.ransackable_attributes(_auth_object = nil)
    %w[created_at description id type_name type_number updated_at]
  end
  ...
  validates :type_number, presence: true, uniqueness: true,
                          format: { with: /\A[1-3,9]\d{3}\z/, message: "must be in the format 'XXXX' where X is a digit" }
  ...
  def validate_type_number_category
    ...
    return if CATEGORY_CODES.values.include?(category_code)
